...
def get_genes_graph(genes_path, save_path, method='pearson', thresh=0.95, p_value=False):
    ...
    genes_exp_df = pd.read_csv(os.path.join(genes_path, 'exp_we_706.csv'), index_col=0)
...
def save_cell_graph(genes_path, save_path):
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    exp = pd.read_csv(os.path.join(genes_path, 'exp_we_706.csv'), index_col=0)
    cn = pd.read_csv(os.path.join(genes_path, 'cn_we_706.csv'), index_col=0)
    mu = pd.read_csv(os.path.join(genes_path, 'mu_we_706.csv'), index_col=0)
    ...
    np.save(os.path.join(save_path, 'cell_feature_cn_std_we.npy'), cell_dict)
    print("finish saving cell mut data!")


def get_STRING_graph(genes_path, thresh=0.95):
    save_path = os.path.join(genes_path, 'edge_index_PPI_we_{}.npy'.format(thresh))
    print('os.path.exists(save_path)',os.path.exists(save_path))
    if not os.path.exists(save_path):
        # gene_list
        # exp = pd.read_csv(os.path.join(genes_path, 'exp.csv'), index_col=0)
        exp = pd.read_csv(os.path.join(genes_path, 'exp_we.csv'), index_col=0)
        ...
        np.save(
            os.path.join('./data/CellLines_DepMap/CCLE_580_18281/census_706/', 'edge_index_PPI_we_{}.npy'.format(thresh)),
            edge_index)
    else:
...
def get_predefine_cluster(edge_index, save_path, thresh, device):
    save_path = os.path.join(save_path, 'cluster_predefine_PPI_we_{}.npy'.format(thresh))
    if not os.path.exists(save_path):
