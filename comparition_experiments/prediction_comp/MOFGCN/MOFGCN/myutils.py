...
def cross_entropy_loss(true_data: torch.Tensor, predict_data:  torch.Tensor, masked: torch.Tensor):
    ...
    # loss_fun = nn.BCELoss()
    loss_fun = nn.L1Loss()
    loss = loss_fun(predict_data+1e-6, true_data)
    return loss
...
def mask(positive: sp.coo.coo_matrix, negative: sp.coo.coo_matrix, dtype=int):
    ...
    # row = np.hstack(positive.row)
    # col = np.hstack(positive.col)
    data = [1] * row.shape[0]
...
def roc_auc(true_data: torch.Tensor, predict_data: torch.Tensor):
    ...
    # np.savetxt(r'D:\xiang_needread\项目代码（重要）\MOFGCN-main\GDSC\processed_data\npresult2.txt', result2)
    true_data = true_data.detach().cpu().numpy()
    true_data = np.where(true_data >= 0.5, 1, 0)
    aucs = roc_auc_score(true_data, predict_data.detach().cpu().numpy())
    return aucs
...
def exp_similarity(tensor: torch.Tensor, sigma: torch.Tensor, normalize=False):
    """
