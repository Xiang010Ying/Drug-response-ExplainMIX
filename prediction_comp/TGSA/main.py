...
def arg_parse():
    ...
    parser.add_argument('--seed', type=int, default=42,
                        help='seed')
    parser.add_argument('--device', type=str, default=torch.device('cuda' if use_cuda else 'cpu'),
                        help='device')
    parser.add_argument('--model', type=str, default='TGDRP', help='Name of the model')
    parser.add_argument('--batch_size', type=int, default=68,
                        help='batch size (default: 128)')
    parser.add_argument('--lr', type=float, default=0.0001,
                        help='learning rate')
    parser.add_argument('--layer_drug', type=int, default=3, help='layer for drug')
    parser.add_argument('--dim_drug', type=int, default=512, help='hidden dim for drug')#128
    parser.add_argument('--layer', type=int, default=3, help='number of GNN layer')
    parser.add_argument('--hidden_dim', type=int, default=3, help='hidden dim for cell')
...
def main():
    ...
    drug_dict = np.load('./data/Drugs/drug_feature_graph_we.npy', allow_pickle=True).item()
    ...
    edge_index = np.load('./data/CellLines_DepMap/CCLE_580_18281/census_706/edge_index_PPI_we_{}.npy'.format(args.edge))
    ...
    args.num_feature = cell_dict['ACH-000425'].x.shape[1]
    ...
    if args.mode == 'train':
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            print('Evaluating...')
            rmse, _, _, _,accuracy, roc_auc,  aupr, precision, recall, f1 = validate(model, val_loader, args.device)
        ...
        train_rmse, train_MAE, train_r2, train_r,accuracy, roc_auc,  aupr, precision, recall, f1 = validate(model, train_loader, args.device)
        val_rmse, val_MAE, val_r2, val_r,accuracy, roc_auc,  aupr, precision, recall, f1 = validate(model, val_loader, args.device)
        test_rmse, test_MAE, test_r2, test_r,accuracy, roc_auc,  aupr, precision, recall, f1 = validate(model, test_loader, args.device)
