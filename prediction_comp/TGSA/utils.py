...
def validate(model, loader, device):
    ...
    prec, reca, _ = precision_recall_curve(np.array(y_t), np.array(y_p))
    aupr = auc(reca, prec)
    ...
    print(
        'acc={:.4f}|auc={:.4f}|aupr={:.4f}|precision={:.4f}|recall={:.4f}|f1={:.4f}'.format(accuracy, roc_auc, aupr,
                                                                                            precision, recall, f1))
    print('y_pred: ', '0:',str(p_0),' ','1:'+str(p_1))
    print('y_true: ',  '0:',str(t_0),' ','1:'+str(t_1))
    return rmse, MAE, r2, r, accuracy, roc_auc,  aupr, precision, recall, f1
...
def inference(model, drug_dict, cell_dict, edge_index, save_name, args):
    ...
    IC = pd.read_csv('./data/PANCANCER_IC_we.csv')
    if args.setup == 'known':
        train_set, val_test_set = train_test_split(IC, test_size=0.2, random_state=42, stratify=IC['Cell line name'])
        val_set, test_set = train_test_split(val_test_set, test_size=2968, random_state=12348,
...
def load_data(IC, drug_dict, cell_dict, edge_index, args):
    if args.setup == 'known':
        # train_set, val_test_set = train_test_split(IC, test_size=0.2, random_state=22, stratify=IC['Cell line name'])
        # val_set, test_set = train_test_split(val_test_set, test_size=0.5, random_state=22,
        #                                      stratify=val_test_set['Cell line name'])

        cell_lines = ['WM115', 'KMRC1', 'MDAMB231', 'DANG', 'LCLC103H', 'LXF289', 'SKCO1', 'PATU8988T', 'CAL851', 'RT112', 'HCC1806', 'BT20', 'SNU61', 'SNU449', 'NCIH196', 'OVCAR4', 'KYSE510', 'RVH421', 'CORL88', 'KMS12BM', 'MDAMB453', 'CAL120', 'MONOMAC6', 'SKES1', 'SBC5', 'UACC62', 'SNU1', 'CAL33', 'NCIN87', 'NCIH747', 'HCC1937', 'SNUC5', 'NCIH2052', 'NCIH661', 'HCC1500', 'IGR37', 'SUIT2', 'OSRC2', 'OV90', 'TE4', 'NCIH2030', 'SKMEL5', 'KMS11', 'NCIH146', 'NCIH2291', 'NALM6']
        drugs = ['JNJ38877605', 'CP724714', 'Talazoparib', 'YM201636', 'S-Trityl-L-cysteine', 'Tenovin-6', 'Brivanib', 'Omipalisib', 'WYE-125132', 'AZD3759', 'CZC24832', 'Mitomycin-C', 'Motesanib', 'SL0101', 'ARRY-520', 'TGX221', 'Cyclopamine', 'SB505124', 'TPCA-1', 'LJI308', 'OSI-930', 'NSC319726', 'VE-822', 'P22077', 'Sepantronium bromide', 'I-BRD9', 'Afuresertib', 'Lapatinib', 'PHA-665752', 'VX-702', 'Elesclomol', 'CMK', 'IOX2', 'Dactolisib', 'Embelin', 'Panobinostat', 'Olaparib', 'Foretinib', 'Ara-G', 'Fluorouracil', 'GSK2578215A', 'EHT-1864', 'AZ960', 'FR-180204', 'AR-42', 'Refametinib', 'Axitinib', 'Obatoclax Mesylate', 'GSK2606414', 'PF-00299804', 'IMD-0354', 'Capivasertib', 'CGP-082996', 'Elephantin', 'Masitinib', 'MK-8776', 'GNE-317', 'BX795', 'GW-2580', 'Etoposide', 'SU11274', 'GSK1059615', 'Lenalidomide', 'AZD6094', 'BMS-754807', 'Afatinib', 'JNK-9L', 'Fulvestrant', 'CUDC-101', 'Temsirolimus', 'GSK690693', 'A-770041', 'BPTES', 'XAV939', 'Trametinib', 'GW843682X', 'Pyrimethamine', 'WEHI-539', 'LFM-A13', 'BX-912', 'Pyridostatin', 'BIBF-1120', 'Y-39983', 'YK-4-279', 'Vinorelbine', 'Avagacestat', 'TWS119', 'OTX015', 'Bosutinib', 'BAY-61-3606', 'T0901317', 'Entinostat', 'WH-4-023', 'Niraparib', 'Tubastatin A', 'Ispinesib Mesylate', 'PLX-4720', 'TW 37', 'Tanespimycin']

        test_index = []
        for i in range(IC.shape[0]):
            if (IC.loc[i,'Cell line name'] in cell_lines) and (IC.loc[i,'Drug name'] in drugs):
                test_index.append(i)
        test_set = IC.loc[test_index,:]
        train_index = []


        for i in range(IC.shape[0]):
            #if i not in test_index and IC.loc[i,'Cell line name'] not in cell_lines:
            if i not in test_index and IC.loc[i, 'Drug name'] not in drugs and IC.loc[i,'Cell line name'] not in cell_lines:
                train_index.append(i)
        train_set = IC.loc[train_index,:]
        ...
        smiles_list = pd.read_csv('./data/Drugs/drug_smiles_we.csv')[
            ['CanonicalSMILES', 'drug_name']]
...
def get_idx_split_drug(dataset, k_splits=5):
    ...
    smiles_name_list = pd.read_csv('./data/Drugs/drug_smiles_we.csv')[['CanonicalSMILES', 'drug_name']]
...
def load_graph_data_SA(args):
    ...
    drug_id2graph_dict = np.load('./data/Drugs/drug_feature_graph_we.npy', allow_pickle=True).item()
    # cell_name2feature_dict = np.load('./data/CellLines_DepMap/CCLE_580_18281/census_706/cell_feature_all.npy',
    #                                     allow_pickle=True).item()
    cell_name2feature_dict = np.load('./data/CellLines_DepMap/CCLE_580_18281/census_706/cell_feature_cn_std_we.npy',
                                     allow_pickle=True).item()
    # drug_name = pd.read_csv("./data/Drugs/drug_smiles.csv").iloc[:, 0]
    drug_name = pd.read_csv("./data/Drugs/drug_smiles_we.csv").iloc[:, 0]
    ...
    cell_feature_edge_index = np.load(
        './data/CellLines_DepMap/CCLE_580_18281/census_706/edge_index_PPI_we_{}.npy'.format(args.edge))
